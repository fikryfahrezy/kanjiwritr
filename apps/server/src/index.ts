...
if (!process.env.CREDENTIAL_SECRET) {
  console.warn("CREDENTIAL_SECRET is unset; pending pairing codes will expire when the server restarts.");
}
if (!rateLimitsEnabled) {
  console.warn("Rate limits are disabled. Do not use this setting in production.");
}
...
const server = Bun.serve<SocketData>({
  ...
  async fetch(request, bunServer) {
    ...
    if (url.pathname === "/api/pairings" && request.method === "POST") {
      if (!limiter.allow(`pair:${clientAddress}`, 8, 60 * 60_000)) return apiError(429, "rate_limited");
      return apiJson(store.createPairing(), 201);
    }

    if (url.pathname === "/api/pairings/claim" && request.method === "POST") {
      if (!limiter.allow(`claim:${clientAddress}`, 20, 10 * 60_000)) return apiError(429, "rate_limited");
    ...
    if (url.pathname === "/api/pairings/current" && request.method === "DELETE") {
      const device = authenticateHttp(request);
      if (!device) return apiError(401, "unauthorized");
    ...
    if (url.pathname === "/ws") {
      ...
      if (!limiter.allow(`ws:${clientAddress}`, 40, 60_000)) return apiError(429, "rate_limited");
  ...
  websocket: {
    ...
    open(socket) {
      const pairingSockets = connections.get(socket.data.pairingId) ?? new Set();
    ...
    message(socket, payload) {
      ...
      if (message.type === "text.deliver") {
        ...
        if (previous) {
          send(socket, { type: "delivery.result", messageId: message.messageId, ...previousResult(previous) });
          return;
        }
        ...
        const timeout = setTimeout(() => {
          ...
          rememberResult(deliveryKey, { delivered: false, error: "delivery_timeout" });
          send(socket, {
            type: "delivery.result",
            messageId: message.messageId,
            delivered: false,
...
function sendToRole(pairingId: string, role: DeviceRole, message: ServerMessage): void {
  for (const socket of connections.get(pairingId) ?? []) {
    if (socket.data.role === role) send(socket, message);
  }
}

function firstSocket(pairingId: string, role: DeviceRole): Bun.ServerWebSocket<SocketData> | undefined {
  return [...(connections.get(pairingId) ?? [])].find((socket) => socket.data.role === role);
}
...
function rememberResult(key: string, result: Omit<DeliveryResult, "expiresAt">): void {
  completedDeliveries.set(key, { ...result, expiresAt: Date.now() + 5 * 60_000 });
}

function previousResult(result: DeliveryResult): Omit<DeliveryResult, "expiresAt"> {
  return {
...
async function parseSmallJson(request: Request): Promise<Record<string, unknown> | undefined> {
  if (Number(request.headers.get("content-length") ?? 0) > 1_024) return undefined;
  try {
...
function fileResponse(file: Blob, pathname: string, headOnly: boolean): Response {
  const immutableAsset = pathname.startsWith("/assets/") || pathname.startsWith("/models/") || pathname.startsWith("/ort/");
