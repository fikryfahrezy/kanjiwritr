...
const defaultServerUrl =
  process.env.KANJIWRITR_DEFAULT_SERVER_URL ?? "http://localhost:3000";
const packageMetadata = (await Bun.file(
  resolve(root, "package.json"),
).json()) as { version?: unknown };
const extensionVersion =
  process.env.KANJIWRITR_EXTENSION_VERSION ?? packageMetadata.version;
...
if (requestedBrowser && selectedBrowsers.length === 0) {
  console.error(
    `Unknown browser "${requestedBrowser}". Use chromium or firefox.`,
  );
  process.exit(1);
}
...
if (
  ...
  !isBrowserVersion(extensionVersion)
) {
  console.error(
    `Invalid extension version "${String(extensionVersion)}". Use one to four dot-separated integers between 0 and 65535.`,
  );
  process.exit(1);
}
...
for (const browser of selectedBrowsers) {
  ...
  const manifest = (await Bun.file(
    resolve(source, browser.manifest),
  ).json()) as Record<string, unknown>;
  manifest.version = extensionVersion;

  await Promise.all([
    writeFile(
      resolve(browserOutput, "manifest.json"),
    ...
    cp(resolve(source, "icons"), resolve(browserOutput, "icons"), {
      recursive: true,
    }),
  ]);
