...
function stroke(id: string, x: number, y: number): InkStroke {
  return {
    ...
    points: [{ x, y, pressure: 0.7, time: 1 }, { x: x + 0.02, y: y + 0.02, pressure: 0.8, time: 2 }],
  };
}
...
describe("handwriting segmentation", () => {
  test("groups multiple strokes by cell and orders multiple lines", () => {
    const groups = segmentStrokes([
      stroke("line-two", 0.05, 0.60),
      stroke("character-two-a", 0.30, 0.10),
      stroke("character-one", 0.05, 0.10),
      stroke("character-two-b", 0.32, 0.12),
    ], 448, 224);

    expect(groups.map((group) => group.strokes.map((item) => item.id))).toEqual([
      ["character-one"],
      ["character-two-a", "character-two-b"],
      ["line-two"],
    ]);
  });
  ...
  test("combines ranked character candidates into sentence beams", () => {
    const groups: RecognizedGroup[] = [
      { key: "0:0", candidates: [{ character: "日", confidence: 0.8 }, { character: "目", confidence: 0.2 }] },
      { key: "0:1", candidates: [{ character: "本", confidence: 0.7 }, { character: "木", confidence: 0.3 }] },
    ];
  ...
  test("orders a long four-line manuscript", () => {
    ...
    expect(groups.map((group) => group.key)).toEqual(Array.from({ length: 20 }, (_, index) => `${Math.floor(index / 5)}:${index % 5}`));
  });
});
