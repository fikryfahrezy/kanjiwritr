...
export type ClientMessage =
  ...
  | { type: "delivery.ack"; messageId: string; delivered: boolean; error?: DeliveryError };

export type ServerMessage =
  | { type: "connection.ready"; connectionId: string; role: DeviceRole; paired: boolean }
  | { type: "pong" }
  | { type: "pairing.completed" }
  | { type: "presence.changed"; extensionOnline: boolean }
  | { type: "text.delivery"; messageId: string; text: string }
  | { type: "delivery.result"; messageId: string; delivered: boolean; error?: DeliveryError }
...
export function parseClientMessage(value: unknown): ClientMessage | undefined {
  ...
  if (
    ...
    return { type: "text.deliver", messageId: parsed.messageId, text: parsed.text };
  }
  if (
...
export function parseServerMessage(value: unknown): ServerMessage | undefined {
  ...
  if (
    ...
    return { type: "connection.ready", connectionId: parsed.connectionId, role: parsed.role, paired: parsed.paired };
  }
  if (parsed.type === "presence.changed" && typeof parsed.extensionOnline === "boolean") {
    return { type: "presence.changed", extensionOnline: parsed.extensionOnline };
  }
  if (
    ...
    return { type: "text.delivery", messageId: parsed.messageId, text: parsed.text };
  }
  if (
