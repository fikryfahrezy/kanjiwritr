...
10-minute pairing code plus its own 256-bit device token. The iPad consumes the
code once through `POST /api/pairings/claim` and receives a different 256-bit
token. Codes are stored as keyed SHA-256 hashes; device tokens are stored as
SHA-256 hashes. Neither credentials nor delivered Japanese text are logged.
...
then routes `text.deliver` only from an iPad to an extension in that pairing.
The extension acknowledges insertion with `delivery.ack`; only then does the
iPad receive `delivery.result`. Results are retained without text for five
minutes to make same-ID retries idempotent.
