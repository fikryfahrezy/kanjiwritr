...
if (!claimResponse.ok)
  throw new Error(`Pairing claim failed (${claimResponse.status})`);
const claim = (await claimResponse.json()) as { token: string };

let resolvePresence!: () => void;
const presence = timeout(
...
const ipadSocket = await openSocket(claim.token, (message) => {
  if (message.type === "presence.changed" && message.extensionOnline)
    resolvePresence();
  if (message.type === "delivery.result" && message.delivered)
    resolveDelivery();
});
await presence;
...
await Bun.sleep(50);
if (!pairingCompleted)
  throw new Error("Extension did not receive pairing completion");
if (unrelatedDelivery)
  throw new Error("Delivery crossed into an unrelated pairing");
...
if (!revokeResponse.ok)
  throw new Error(`Revocation failed (${revokeResponse.status})`);
await Promise.all([closeA, closeIpad]);
unrelatedSocket.close();

console.info(
  "Pairing, presence, isolated delivery, acknowledgement, and revocation passed.",
);
